Tascuri pe echipa:

Alexandru
1. Crearea unei hărți mini (mini-map simplă 2D). (done)
2. Adăugarea unui sistem de particule vizual (stele, scântei).
3. Sistem de scor (crește la fiecare monedă).
4. Adăugarea unui sistem de level-uri (ex: Level 1 → Level 2).
5. New Atack

Aruni
1. Designul UI (score text, butoane, ecran Game Over).
2. Crearea unui logo și a unui ecran de splash (intro).
3. Checkpointuri
4. Cod pentru pauză în joc (meniu „Pause” cu Resume/Quit).
5. New enemy
